...
const Header: React.FC<HeaderProps> = ({ contact, socials, locale }) => {
    ...
    const tel = "+994 77 331 26 53 / +90 552 153 23 28";
    const email = "info@neymantech.com";

    if (isProductPage) {
        return null; // Global header hidden on dynamic product pages
    }
                    ...
                    <div className="flex items-center justify-between py-4">
                        ...
                        <nav className="hidden lg:flex items-center justify-center gap-8 lg:w-2/4">
                            {navLinks.map((link: NavLink) => (
                                ...
                                >
                                    {navT(link.translationKey)}
                                </Link>
                            ))}
                        </nav>

                        {/* CTA & Language - Right */}
                        <div className="hidden lg:flex items-center justify-end lg:w-1/4 gap-6">
                            <LanguageSwitcher currentLocale={locale} />
                            <MotionLink
                                href={`/${locale}/contact`}
                                className="group flex items-center gap-2.5 bg-blue-600 text-white px-6 py-2.5 rounded-xl font-bold text-[14px] tracking-wide hover:bg-blue-700 shadow-md shadow-blue-600/20"
